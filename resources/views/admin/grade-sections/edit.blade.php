    ...
    <div class="py-12">
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.grade-sections.update', $gradeSection) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="grade_level" id="grade_level" required
                                ...
                                @foreach($gradeLevels as $level)
                                    <option value="{{ $level }}" {{ old('grade_level', $gradeSection->grade_level) == $level ? 'selected' : '' }}>
                                        {{ $level }}° Grado
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="section" id="section" required
                                ...
                                @foreach($sections as $sec)
                                    <option value="{{ $sec }}" {{ old('section', $gradeSection->section) == $sec ? 'selected' : '' }}>
                                        Sección {{ $sec }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="school_year_id" id="school_year_id" required
                                ...
                                @foreach($schoolYears as $year)
                                    <option value="{{ $year->id }}" {{ old('school_year_id', $gradeSection->school_year_id) == $year->id ? 'selected' : '' }}>
                                        {{ $year->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6 border-t border-gray-200 pt-6">
                            <div class="flex items-center gap-3 mb-4">
                                <label for="use_global_break" class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" id="use_global_break" name="use_global_break"
                                        value="1" {{ !$gradeSection->break_time_start ? 'checked' : '' }}
                            ...
                            <div id="break-time-fields" class="{{ $gradeSection->break_time_start ? '' : 'hidden' }} grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="time" name="break_time_start" id="break_time_start"
                                        value="{{ old('break_time_start', $gradeSection->break_time_start) }}"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <div>
                                    ...
                                    <input type="time" name="break_time_end" id="break_time_end"
                                        value="{{ old('break_time_end', $gradeSection->break_time_end) }}"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
