<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            Registrar Disponibilidad de Docente
        </h2>
    </x-slot>
            ...
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="border-b border-gray-200 bg-white px-4 py-5 sm:px-6">
                    <h3 class="text-lg font-medium leading-6 text-gray-900">
                        Nueva Disponibilidad de Docente
                    </h3>
                    <p class="mt-1 max-w-2xl text-sm text-gray-500">
                        Define las franjas horarias en que cada docente está disponible para impartir clases.
                    </p>
                </div>

                <form action="{{ route('admin.teacher-availabilities.store') }}" method="POST" class="p-6">
                    @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="note" id="note" rows="3"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                placeholder="Ej: Solo disponible en la tarde, horario sujeto a cambios, etc.">{{ old('note') }}</textarea>
                            @error('note')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
