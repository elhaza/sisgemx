    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="mb-6 overflow-hidden rounded-lg bg-white shadow-sm">
                <div class="p-6">
                    <form method="GET" action="{{ route('teacher.medical-justifications.index') }}" class="grid gap-4 md:grid-cols-5">
                        <div>
                        ...
                        <div>
                            <label for="from_date" class="block text-sm font-medium text-gray-700">Desde</label>
                            <input type="date" name="from_date" id="from_date" value="{{ request('from_date') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        </div>
                        <div>
                            <label for="to_date" class="block text-sm font-medium text-gray-700">Hasta</label>
                            <input type="date" name="to_date" id="to_date" value="{{ request('to_date') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        </div>
                        ...
                        @if(request()->hasAny(['status', 'from_date', 'to_date']))
                            <div class="flex items-end">
                                <a href="{{ route('teacher.medical-justifications.index') }}" class="inline-flex items-center rounded-md bg-gray-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-gray-700">
                                    Limpiar Filtros
                                </a>
                            </div>
                        @endif
                                    ...
                                    @foreach($medicalJustifications as $justification)
                                        <tr>
                                            <td class="whitespace-nowrap px-6 py-4">
                                                ...
                                                <div class="text-sm text-gray-500">{{ $justification->student->grade_level }}</div>
                                            </td>
                                            ...
                                            <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                                <a href="{{ route('teacher.medical-justifications.show', $justification) }}" class="text-blue-600 hover:text-blue-800">
                                                    Ver Detalles
                                                </a>
                                            </td>
                                        </tr>
